    ...
    if (exception_setup(true)) {
        for (int r = 0; ok && r < reps; r++) {
            ...
            if (rval) {
                ...
                /*
                 * Check whether padding in array removes are still initial
                 * value 'X'. If there's other character in padding, it's
                 * overflowed.
                 */
                ...
                if (!check && fail_count < fail_limit) {
                    ...
                    report(
                        1,
                        "ERROR: Removal from queue failed (%d failures total)",
                        fail_count);
                    ok = false;
                }
            }

            if (reps == 1 && ok && check && strcmp(removes, checks)) {
                report(1, "ERROR: Removed value %s != expected value %s",
                       removes, checks);
                ok = false;
            }
